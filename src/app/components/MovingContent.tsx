...
const ProjectContent = ({ projects }: { projects: ProjectsArray }) => {
  ...
  const [isBottomVisible, setIsBottomVisible] = useState(false); // Stan widoczności obrazu
  const projectRefs = useRef<(HTMLDivElement | null)[]>([]);
  const bottomRef = useRef<HTMLDivElement | null>(null);

  const handleScroll = () => {
    const scrollPosition = window.scrollY + window.innerHeight / 2; // Środek ekranu
    ...
    projectRefs.current.forEach((project, index) => {
      if (project) {
        ...
        if (scrollPosition >= projectTop && scrollPosition < projectTop + projectHeight) {
          currentActiveProject = projects[index].id; // Aktywny projekt
        }
      }
    });
    ...
    // Sprawdzamy, czy dolna sekcja jest widoczna
