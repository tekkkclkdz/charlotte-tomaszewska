...
            else if (isMobile.current && index !== -1 && !entry.isIntersecting)
...
              className="relative flex items-center py-4 justify-center bg-white text-black font-customMedium cursor-pointer z-0 mb-0"
              id={`project${project.id}`}
            >
              {loadedProjects.has(project.id) ? (
                <div className="text-center">{project.content}</div>
              ) : (
...
          <h2 className="relative text-lg sm:text-2xl mb-0 sm:-mb-4 font-customMedium mix-blend-difference text-black dark:text-white opacity-100 transition-opacity duration-300">
